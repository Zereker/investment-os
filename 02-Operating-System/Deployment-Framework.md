...
本框架用于 Transition Mode 下历史超额现金的迁移。每月固定投入与战略基线是例行执行；超过基线的金额才属于战术加速。
...
- \(S=\max(C-15\%\times V,0)\)：相对现金目标的战略剩余；
- \(G\)：分配 \(D\) 后 SPYM 与 QQQM 的正缺口合计。
...
当 Valuation Score 为 `N/A` 时，战术加速金额 \(T=0\)，但固定投入与战略基线仍可执行。
...
- 固定投入与 \(B\) 优先流向正缺口更大的标的；可按缺口比例分配或只买缺口最大的 1–2 项。
...
账户层同时披露 \(V,C_0,D,C,R,S,G,B,T\)、现金比例、融资借款和未完成订单。
