...
- \(F\)：本月已到账的实际外部净入金，且 \(F\ge0\)；计划值为 2,000 美元，提款或未到账计划额不得计入 \(F\)；
- \(V\)：\(F\) 到账后、交易前的账户净值；
- \(C_0\)：包含 \(F\)、全部例行订单前的投资组合现金；
- \(G_0\)：按入金后 \(V\) 与动态目标计算、Routine DCA 前 SPYM 与 QQQM 的正缺口合计；
...
- \(S=\max(C-15\%\times V,0)\)：Routine DCA 后相对 15% 现金目标的战略剩余。
...
- 按 Constitution 分别计算 QQQM 28% 与 SPYM `57%−A` 的正缺口。
