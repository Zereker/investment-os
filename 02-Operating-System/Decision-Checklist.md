...
任何非例行买入、卖出、换仓、新 Alpha 或超额现金战术部署，都必须在下单前完成本清单。不得在成交后补写理由。

每月固定投入仍可走月度流程，但同样必须通过实时数据、目标缺口、现金和订单冲突检查。
...
- [ ] 标的价格、币种、数量和时间戳已确认
- [ ] 账户接口之间不存在未解释冲突
- [ ] 依赖的市场与估值字段通过 `08-Data/DATA_QUALITY.md`
...
- 是否突破 Cash、Alpha 合计或单一 Alpha 上限？
- 与 QQQM 或现有 Alpha 是否形成重复暴露？
...
- 新 Alpha 会使单只超过总组合 6%或 Alpha 合计超过 15%。
