...
- SPYM / QQQM 按 `ETF-Valuation-Framework.md` 生成的估值等级（v4.0 起只影响 `B / T`）
...
- \(B\) 在非`VERY EXPENSIVE`标的之间按剩余正缺口分配；为减少碎片交易，可只购买缺口最大的1–2项。
...
每次 \(T>0\)、回撤部署、倾斜动作、卖出或公式例外，Journal 至少记录：
