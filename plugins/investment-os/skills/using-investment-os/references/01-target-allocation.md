...
2. 当季 `08-lookthrough-check.md` 手工核查有效；过期或 `DATA INCOMPLETE` 即冻结回补；
...
**提高倾斜仍须完整 IC**，`04-position-registry.md` 的五步闸门不变。回补上限 `min(A_execution_cap, A_stage)` 不因回补走例行路径而放宽；本条不新增任何资金、不放宽任何上限。（依据见 [source repository research](https://github.com/Zereker/investment-os/blob/master/Research/2026-08-01-soxx-restore-vs-increase.md)）
...
组合的合并穿透暴露以季度手工核查计算（方法见 `08-lookthrough-check.md`），使用各基金最新官方持仓/行业表，合并直接持仓与 ETF 内含持仓。
...
本条款动用的是常态现金下限**以下**的结构性现金，即危机弹药——从 15% 现金目标算起合计 9 个百分点的 NAV（降至 T4 的 `6%+U`，量化见 [source repository research](https://github.com/Zereker/investment-os/blob/master/Research/2026-08-01-drawdown-deployment-drill.md)）。弹药有限，因此触发必须是**广谱市场信号**；SPYM 代表美国大盘，是本组合的广谱代表。
