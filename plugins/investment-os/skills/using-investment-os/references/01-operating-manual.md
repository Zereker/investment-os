...
`skills/validating-drawdown-state/scripts/drawdown_drill.py` 用 SPYM 十年真实收盘重放本状态机，七项不变量全部成立（记录见源码仓库研究档案）。**已验证**：价格 → 档位判定、每档每周期一次、由浅到深、新高重置、确定性重放。**未验证**：本手册状态重建部分第 4 步「本周期各档是否已执行」的三信号交叉重建——它依赖 Journal、IBKR 成交与警报指针，无法离线验证，在真实周期跑过之前保持未验证状态。
