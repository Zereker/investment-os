...
完全符合 `Monthly-Workflow.md` 的固定投入、战略现金迁移基线、按档执行的回撤部署与按五项约束执行的 SOXX **回补至目标**走例行路径；一旦偏离公式、标的、档位或上限，立即升级为本清单。二者的判别只看 `A_execution_cap` 动没动（定义见 Constitution）。
...
- [ ] 标的价格、币种、数量和时间戳已确认
- [ ] 账户接口之间不存在未解释冲突
- [ ] 依赖的市场与穿透字段通过 `08-data-quality.md`
...
- 当季`LOOKTHROUGH_CHECK.md`核查缺失、过期或结论为`DATA INCOMPLETE`时申请任何倾斜买入（提高倾斜与回补同样冻结）。
...
最终下单由账户所有者在 IBKR 中亲手确认。非例行决定必须先写入 `Decision-Log.md` 或 `05-investment-journal.md`；每日复盘或 Investment Committee 的结论都不等于自动下单授权。
