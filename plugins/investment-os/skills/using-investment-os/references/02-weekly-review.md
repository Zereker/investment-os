...
- 最新有效的 `08-YYYY-MM-DD-lookthrough-check.md` 数据
- `04-position-registry.md`
- 本周成交、未完成订单和异常记录
- 当前 `03-transition-plan.md` 的阶段状态
...
周度复盘只标记偏差，不因一周波动临时再平衡。SOXX持有状态本身不产生`IC REVIEW`；**提高倾斜**候选只有满足 Position Registry 提高倾斜闸门（当季手工核查 + 实时账户 + 完整IC）才可进入`IC REVIEW`。**回补至目标**走月度例行路径，不产生`IC REVIEW`，只在月度流程第 4 步评估。
