...
| Look-through Concentration | IBKR 组合权重 + 官方行业/持仓表 + `08-lookthrough-check.md`手工核查 | `skills/routing-investment-research/scripts/fetch_etf_data.py`（辅助计算） | 每季度及倾斜追加前 | Green（Derived） | 手工加权求和并存档；缺失或口径不明时倾斜追加冻结 |
