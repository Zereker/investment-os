# Look-through Evidence Bundle v1.5
...
仓库不维护中央 issuer / GICS 数据库。身份、分类和 ETF 持仓在运行时从分别登记的专业来源取得；普通巡检不写仓库。只有形成真实决策证据时，才将当次多源结果及其来源、`as_of`、原始文件和哈希写入 Bundle。

所有 Bundle 文件只增不改。CI 以 PR base 或 push 前一提交为基点检查历史快照；来源、映射、账户或候选情景变化时必须创建新 Bundle。

Bundle 顶层和 `raw/` 必须与契约引用精确一致；未被 Packet 绑定的影子 Packet、额外来源、子目录或其他文件都会失败。历史检查必须能够读取完整 base commit；base 不可用时失败关闭，不能静默跳过。

## Packet 必填结构

- `schema_version`：当前固定为 `1.5`；验证器继续支持历史 `1.4` Bundle 重放。
...
- `issuer_registry_path / issuer_registry_sha256`：固定绑定 `issuer-registry.json`。每个来源 ID 先解析为 `canonical_security_id`，再绑定唯一 CIK 或 LEI；不同 ID 与股权类别必须共用同一法律实体身份。每条身份记录保存已登记来源与 `as_of`。
- `mapping_path / mapping_sha256`：指向同一 Bundle 内的统一行业映射快照。每条记录保存已登记分类来源与 `as_of`；缺少管理人原始分类时，必须使用独立权威分类来源，不能由 Bundle 自述降低风险。
...
发行人注册表与行业映射中的每条记录都必须保存结构化 `source_url / as_of` 证据；自由文本不满足证据契约。发行人身份只允许 `cik:<10-digits>` 或 `lei:<20-character-LEI>`；CIK 必须绑定同一编号的 SEC URL，LEI 必须绑定同一编号的 GLEIF URL。注册表拒绝重复发行人 ID、重复规范公司名和重复来源 ID；每个 SEDOL、CUSIP、ISIN 或 ticker alias 必须指向同一快照中已存在、且自指的 `canonical_security_id`，alias 与 canonical 必须共用发行人。因此跨基金的 SEDOL↔CUSIP 以及 Alphabet A/C 都按同一实体累计。衍生品映射中的每个组件必须同时存在于同一映射快照并直接解析为普通证券；缺失、嵌套或循环组件不能进入 Green。可识别的管理人原始 Sector / Industry 必须与统一映射一致；原始分类缺失时必须有独立权威分类来源，否则进入分类未知。
...
2. 官方产品 URL、原始文件实哈希、确定性解析结果、已登记身份/分类来源、Bundle 快照、账户、Candidate 与 Packet 哈希全部通过；
