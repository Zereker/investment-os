...
Look-through Bundle v1.5区分数据失败与政策失败：

- 官方来源、发行人或分类覆盖不完整：`DATA INCOMPLETE`
- 数据覆盖完整，但一个或多个组合护栏未通过：`POLICY GATE FAIL`
- 数据覆盖完整且所有护栏通过：`DATA GATE PASS`

IBKR账户中的直接证券必须在`direct_holdings`逐项列示，其市值合计与
`current_market_values.other`一致，再进入同一issuer/GICS计算。严格识别的现金、
货币市场基金与公司行动`CONTRA`结算占位保留在原始对账中，但不计入issuer/GICS
经济敞口。
