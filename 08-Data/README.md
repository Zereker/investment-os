...
- `LOOKTHROUGH_CHECK.md`：季度穿透手工核查程序与记录模板（v4.0 起取代 Bundle 验证器）。
- `SNAPSHOTS/`：按观察日期保存不可变的官方估值快照与季度核查记录。
...
按 `LOOKTHROUGH_CHECK.md` 手工完成，记录存入 `SNAPSHOTS/`。核查结论只限制自主倾斜新增或触发复核，不自动改变 Registry、不授权交易。
