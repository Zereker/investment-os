...
look-through exposure for the quarterly manual check (08-Data/LOOKTHROUGH_CHECK.md).
...
Usage:
  ...
  python3 scripts/fetch_etf_data.py --scenario current --markdown   # snapshot block for 08-Data/SNAPSHOTS/
...
def main() -> None:
    ...
    ap.add_argument("--markdown", action="store_true", help="emit snapshot block for 08-Data/SNAPSHOTS/")
    args = ap.parse_args()
    ...
    if weights:
        ...
        print("   信息技术: 用管理人官方行业表手工计算（本工具持仓表不含行业列）——见 LOOKTHROUGH_CHECK.md 第2步")
        ...
        if args.markdown:
            print("\n---8<--- 粘贴到 08-Data/SNAPSHOTS/YYYY-MM-DD-lookthrough-check.md ---")
