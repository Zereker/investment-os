...
(01-Constitution/Target-Allocation.md). That clause had never been executed —
...
executed" half of 02-Operating-System/State-Reconstruction.md, which depends on
...
per 08-Data/DATA_REGISTRY.md. Adequate for a drill; the production trigger must
...
Usage:
  python3 scripts/drawdown_drill.py                    # 10Y SPYM replay + invariant checks
  python3 scripts/drawdown_drill.py --symbol spy       # cross-check against the index proxy
  python3 scripts/drawdown_drill.py --range 5Y
  python3 scripts/drawdown_drill.py --markdown         # block for a Research note
"""
