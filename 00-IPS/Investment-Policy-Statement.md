...
基准只在每个自然月首个估值时点重置为15% / 57% / 28%。月内各袖套随自身收益漂移，不因每日净值变化再平衡。
...
SPYM 与 QQQM 使用含分红总收益。`r_cash_model`必须以月初“基准净值的15%假设USD现金余额”为本金，按同期IBKR官方公布的账户计划、NAV比例、免息门槛、币种、Segment和利率按月计息（公式见 `08-Data/DATA_DICTIONARY.md`）；月内不得把现金本金重置为每日基准净值的15%，利息不在月内复利。不得使用实际账户约束下的单位现金收益率，也不得把实际利息美元映射到基准。
