...
本条款让结构性现金在严重回撤中可以真正部署，且不依赖任何估值数据：
...
- 部署走月度例行路径的全部数据与执行检查（实时 IBKR、无融资、无冲突订单、只买正缺口），但**不受估值等级与估值数据可得性约束**；回撤部署优先于估值暂停。
...
1. Routine DCA 与战略基线按 `A_basis` 和 `U` 计算，只进入 SPYM / QQQM 正缺口；`ETF-Valuation-Framework.md` 只按 v4.0 降级后的职责影响 `B / T`，不得阻塞 `D`，不得创造负缺口或突破目标上限。
...
6. `EXPENSIVE / VERY EXPENSIVE` 只按降级后职责影响 `B / T`；估值等级本身不构成卖出理由。
