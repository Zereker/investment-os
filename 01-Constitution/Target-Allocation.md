...
每档释放 **1.5pp of NAV** 的**定额**，只进入 SPYM / QQQM 正缺口；六档合计 9pp，从 15% 现金目标降至绝对下限 `6%+U`。**绝对下限 `6%+U` 是硬停，任何情形不得突破。**
...
- v4.3 曾存在的「T1 在 10% 消耗后，10%–25% 区间无额外解锁」这一代价，已由 v4.4 的六档阶梯消除——该区间现由 T2（15%）、T3（20%）、T4（25%）逐档覆盖。触发线下调的取舍与证伪回路仍见 `Research/2026-08-01-t1-threshold-10pct.md`。

### 为什么触发口径只用 SPYM

本条款动用的是常态现金下限**以下**的结构性现金，即危机弹药——从 15% 现金目标算起合计 9 个百分点的 NAV（降至最深档的 `6%+U`，量化见 `Research/2026-08-01-drawdown-deployment-drill.md`）。弹药有限，因此触发必须是**广谱市场信号**；SPYM 代表美国大盘，是本组合的广谱代表。
