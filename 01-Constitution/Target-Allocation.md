...
**`DD` 超过 25% 后不再解锁任何档位。** 弹药在 T4 处打光，现金停在 `0+U`，此后只用外部新增资金逐月重建。继续下跌**不产生**新的部署授权，也**不得借款加仓**——这不是遗漏，是 v4.6 的明确决定（依据见 `Research/2026-08-01-drawdown-four-tier.md`）。

关键在「定额」二字：v4.3 及之前的写法是「把下限以上的现金全部部署」，这会在首档就把整段一次打完——那不是分批。改为定额释放后，**跌得越深买得越多，且每笔落在不同价位**。本条款不判断谷底，只按跌幅机械递增。（依据见 `Research/2026-08-01-drawdown-tranching.md`）
...
梯度把 60% 的弹药压在 T3/T4——跌得更深、价格更好的位置，同时把首笔压到 1.5pp。等额方案的首笔会是 3.75pp，落在 `DD` 10% 这个证据显示的最差入场点上（见 `Research/2026-08-01-t1-threshold-10pct.md`）。
...
代价已量化：放弃更深的入场点使每单位资金的「到收复新高」回报由六档的 24.1% 降至 22.5%（2020 周期加权入场价贵 2.8%）；梯度分配把它补回到 26.7%。
