# SOXX Investment Thesis

- Production classification: `板块倾斜（Sector Tilt）`
- Role: 唯一半导体倾斜 ETF，替代 MU、TSM 等分散的单股主动预算
- Policy research status: `Incomplete — INDEX METHODOLOGY EVIDENCE`
- Production lifecycle: 见 `04-Alpha/Position-Registry.md`（当前 `Hold`，追加受閘门约束）
- Permanent hard cap: 6%（v4.0；10%/12.5%/15%历史阶段作废）
- Current execution cap: 3%
- Research date: 2026-07-30；v4.0 更新：2026-07-31

## Thesis

1. 半导体是数字基础设施、AI计算、存储、网络、汽车和工业自动化的关键投入，长期需求具备结构性增长基础。
2. SOXX以30只美国上市半导体及设备证券覆盖设计、制造、存储与设备链条，比重新集中持有少数半导体个股更适合作为唯一Alpha载体。
3. 该观点具有强周期性，不能仅凭行业增长或近期回撤执行；分阶段上限、ETF穿透与组合护栏是Thesis不可分割的一部分。

## 产品与指数

- iShares页面显示SOXX跟踪NYSE Semiconductor Index；基金费用率0.34%，持仓30只。
- 2026-06-30官方Fact Sheet显示3年标准差35.47%、P/E 76.39倍、前十大持仓61.29%，说明ETF仍高度集中且估值敏感。
- 现行NYSE Semiconductor Index完整方法文件尚未从ICE Index Platform取得可审计副本；在核验成分选择、加权、上限和再平衡规则前，不得把旧PHLX SOX方法当作现行方法。

## 正方

- 用行业篮子降低单一公司的产品路线、财务、管理与执行失败风险。
- 覆盖逻辑、存储、设备和代工链条，避免只押注单一子周期。
- 作为唯一Alpha载体，治理边界比多只半导体个股清晰。

## 最强反方

- SOXX并不等于广泛分散：只有30只持仓，前十大超过60%。
- 半导体盈利与资本开支高度周期化；WSTS 2026年90%增长预测主要由约250%的存储增长推动，极端增长本身可能意味着更高的未来均值回归风险。
- QQQM与SPYM已内含大量同类发行人，15% SOXX可能使科技和半导体穿透远高于表面仓位。
- 高估值、高波动和相关性可能在风险事件中同时打击SOXX、QQQM与SPYM。

## 可证伪条件

- 指数或基金结构发生变化，使其无法代表预期的半导体价值链；
- 费用、跟踪质量、流动性或税务显著恶化；
- 长期需求与价值创造不再转化为成分企业盈利和自由现金流；
- 风险、税费与复杂度调整后，滚动五年持续不如等额SPYM且无可信改善依据；
- 组合穿透长期阻止新增，证明倾斜预算与整体IPS不兼容（2026-07实测已据此把上限封顶为6%）。

## 组合损害

- 6%上限下，SOXX下跌50%对组合的直接影响约为-3%，未计QQQM/SPYM同步下跌（合并半导体约24%意味着同步下跌才是主要损害来源）。
- 历史15%端点在同情景下直接影响约为-7.5%，且合并半导体约32%；该端点已依据实测证据作废。

## 上限与交易（v4.0）

- `A_stage=6%`为永久硬上限，当前`A_execution_cap=3%`；执行上限只能按3%→4.5%→6%逐档推进。
- 10%、12.5%、15%历史阶段已作废：2026-07实测合并穿透显示这些端点在系统自身护栏下不可达（见`Research/2026-07-31-v4-Evidence-and-Proposal.md`）。
- 任何追加前必须通过当季`LOOKTHROUGH_CHECK.md`手工核查、实时IBKR、执行上限和完整IC。
- 价格档位仅触发研究复核，不写入Production自动规则。

## 官方来源

- iShares SOXX产品页：https://www.ishares.com/us/products/239705/ishares-phlx-semiconductor-etf
- iShares Fact Sheet（2026-06-30）：https://www.ishares.com/us/literature/fact-sheet/soxx-ishares-semiconductor-etf-fund-fact-sheet-en-us.pdf
- iShares SOXX Summary Prospectus：https://www.ishares.com/us/literature/summary-prospectus/sp-ishares-phlx-semiconductor-etf-3-31.pdf
- ICE Equity Index Methodologies：https://www.ice.com/fixed-income-data-services/index-solutions/equity-indices/methodologies
- ICE Semiconductor Index 2022方法更新通知：https://www.ice.com/publicdocs/equity_indices/notices/ICEBIO_ICESEMI_Methodology_Updates_20220128.pdf
- WSTS Spring 2026 Forecast：https://www.wsts.org/esraCMS/extension/media/f/WST/7618/WSTS_FC-Release-2026-May.pdf

核心投资逻辑已记录，但现行指数方法证据尚未完整，因此Policy research status保持`Incomplete`。该状态与缺少同一审核日采集、三只ETF `source_as_of`一致的Green穿透快照均会使结论保持`WAIT / DATA INCOMPLETE`；本文件不构成交易授权。
