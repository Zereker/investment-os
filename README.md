...
2. **回撤部署**：SPYM 相对历史最高收盘回撤达档时，现金下限分档降至 `10/8/6%+U` 并部署至 Core 正缺口；不依赖任何估值数据，每档每轮回撤周期一次。15% 现金的成本（约 0.45–0.75pp/年）与职能自此对得上账。（v4.0 首档为 ≥15%，v4.3 下调至 ≥10%，见下。）
...
当前生产入口：[PRODUCTION.md](PRODUCTION.md)；版本说明：[v4.3](07-Releases/v4.3.md)（结构性变化见 [v4.0](07-Releases/v4.0.md)）
