...
v3.4.2不改变v3.4策略或v3.4.1可靠性规则：SOXX长期硬上限15%、当前阶段6%、当前执行上限3%。本补丁把穿透Data Gate执行化：新增不可变证据Bundle、归档原始字节哈希、独立映射/账户情景、名义敞口重算与故障注入CI。验证通过只证明数据证据合格，不改变Registry、不创建交易候选。SOXX仍为`Frozen — DATA GATE`，发布不产生交易。
