...
## BUG-022：中央身份与分类表重复维护外部金融数据

- 日期：2026-07-31
- 事件：仓库同时保存全量中央issuer/GICS表和Bundle内同一份决策快照，日常运行被设计成继续追加中央表。
- 影响：重复数据占用仓库、制造人工维护义务，并可能让旧副本被误当作当前权威。
- 根因：把运行时多源数据编排与历史决策留证混成一个持久数据库。
- 修复：删除中央issuer/GICS表；运行时分别读取已登记专业来源；仅在真实决策Bundle中冻结当次身份、分类、来源、日期和哈希。
- 防复发：CI禁止恢复中央authority依赖，只允许历史Bundle只增不改；普通巡检不写仓库。
- 状态：Closed
