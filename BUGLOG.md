...
- 根因：把计息余额计算误写为每日权重重置。
- 修复：月初设置15%现金本金，月内只随利息递推，下月再重置。
- 防复发：自动测试禁止每日现金重置公式，并要求月初状态公式。
- 状态：Closed
...
- 修复：当前统一为`Frozen — DATA GATE`，并定义`Frozen → Approved / Add Candidate → IC APPROVE → 人工下单`。
- 防复发：Registry是唯一状态源；自动测试禁止现行文件出现复合状态。
- 状态：Closed
