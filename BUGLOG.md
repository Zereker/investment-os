...
## BUG-023：新高后警报价格更新但档位指针未重置

- 日期：2026-08-02
- 事件：券商内唯一回撤警报的价格使用了当前 ATH 重新计算，但仍对应旧周期的下一档，而当前周期首档尚未执行。
- 影响：警报可能跳过本周期第一个可用档，导致每日巡检错误理解回撤状态，并在真实触发时漏报或延迟部署检查。
- 根因：ATH 价格基准和档位身份通过不同状态路径维护；新高重置时只更新了价格，没有把已执行档位和 alert pointer 作为同一个原子状态退回首档。
- 修复：新增 `scripts/alert_pointer_check.py`，由 ATH 与三信号重建结果独立计算 expected pointer，再与 IBKR actual alert 比较；更新状态重建与每日巡检规范。
- 防复发：每日强制检查警报数量、标的、字段、运算符、档位和价格；任何差异令 `Account Health = WARN`、回撤部署状态 `DATA INCOMPLETE`，并停止新的回撤部署候选。CI 运行警报指针自测，覆盖新周期首档、已执行一档、重复警报和阶梯耗尽场景。
- 状态：Monitoring
