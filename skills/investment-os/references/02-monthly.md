...
- \(G\)：分配 \(D\) 后的剩余正缺口合计；
- \(R\)：到 2028-12（含）剩余的月度执行次数，**最小为 12**——计划期结束后按 12 个月滚动，使战略剩余始终分批部署，不塌缩成一次性下注；
...
每月用最新数据重算，不沿用旧金额。基线必须同时满足：交易后物理现金不低于 12%（回撤部署档生效时按其临时下限）；不使用融资；资金只进入正缺口；Data Gate、订单冲突和执行检查通过。
