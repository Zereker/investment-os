...
1. 每日巡检**分别**记录 SPYM 与 QQQM 收盘价相对**其各自**历史最高收盘价的回撤 `DD` 与各自的回撤周期状态。SOXX 无阶梯，不记录档位。
2. **逐标的**计算。对每只有阶梯的标的 \(i\)：部署额 \(= \min\bigl(\sum_{j\in K_i} w_{ij}\times V,\ \max(C,0),\ g_i\bigr)\)，其中 \(K_i\) 是该标的本次新触发且其周期内未执行的档集合，\(w_{ij}\) 是该档的梯度定额，\(g_i\) 是**该标的自身**的正缺口。回撤更深的标的先分配现金。
3. 每档在**该标的**同一回撤周期内最多执行一次；该标的创出新的历史最高收盘价后**只重置它自己的**周期，其各档恢复可用。
3b. **达档但部署额为 0 的档位不计为已执行**——该标的无正缺口或无现金时，档位保持可用。档位只有真正买入才消耗，否则与本文件用成交记录重建档位状态的做法自相矛盾。
4. 单日可同时满足多档，按由浅到深顺序逐档执行，仍受「每档一次」限制。
5. **某档释放的资金只买触发它的那只标的**，上限是该标的自身的正缺口；超出部分本周期不部署，**不溢出到其他标的**。分批限价执行。
...
**首次真实触发时**，除按上述步骤执行外，还必须在当前私有会话核验 `05-state.md` 的档位重建过程本身是否给出了正确档位——该步骤依赖 IBKR 成交与现金，无法离线验证。
