...
3. 使用 `python3 skills/investment-os/scripts/alert_pointer_check.py`，比较重建出的 expected pointer 与 IBKR actual alert。
...
`D`、战略迁移、回撤部署和回补的具体公式与阈值由 `00-constitution.md`、本手册部署框架和 `skills/investment-os/scripts/monthly_execution.py` 当前实现共同约束。本文不复制易变参数。
...
- 当前私有会话中的当季穿透核查证据（本流程第一步生成）
- 组合、Policy Benchmark 与两个影子基准（SB-1 / SB-2）的季度及滚动三年数据

### 第一步：穿透手工核查

按 `02-data-contract.md` 穿透核查程序完成本季核查。证据只存在于受信任的私有运行时或当前私有会话，不写入仓库；没有可信证据时必须重算，无法重算则不得给出任何倾斜追加相关结论。
...
7. 每次执行在当前私有会话报告 `DD`、档位、部署额、订单细节、交易后权重与现金；账户与成交事实以券商记录为准，不写入仓库。
...
4. **重建回撤周期状态**:
   ...
   - **各档已执行判定以周期内 IBKR 成交记录为主信号**：识别周期起点之后、由回撤部署产生的 Core 买入，按**金额对应该档梯度定额**且**日期落在 `DD` 达该档之后**归属到具体档位，逐档标记为已执行。归属不确定时 `DATA INCOMPLETE`，不得猜测。
   ...
   - **Alert pointer 一致性检查**:把当前 ATH、重建出的 `tiers_executed` 和 IBKR active alerts 输入 `python3 skills/investment-os/scripts/alert_pointer_check.py`。未耗尽阶梯时必须恰好一个启用警报,且标的、字段、运算符、档位和价格均与 expected pointer 一致;阶梯耗尽后必须没有启用回撤警报。任何差异 ⇒ `Account Health = WARN`、`drawdown deployment state = DATA INCOMPLETE`、停止新的回撤部署候选。agent 只报告 expected/actual 和修复条件,不自动修改券商警报。
5. **穿透核查**:使用当前私有会话或受信任私有运行时中的当季核查证据；过期、缺失或无法验证时重新核查，仍不可得则倾斜追加冻结，例行路径不受影响。仓库不保存核查副本。
...
- 与 SPYM、QQQM 的穿透重复暴露是多少（使用当前私有核查证据）？
