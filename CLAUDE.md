...
本仓库是一套**由 AI 执行的个人投资操作系统**（当前 v4.3）。你（AI）的职责是按已发布规则读取数据、计算、报告和把关——**永远不下单**；下单只能由账户所有者在 IBKR 人工完成。
...
- **只有 QQQM/SOXX 跌而 SPYM 没跌**时不触发回撤部署,由再平衡的正缺口吸收——`D/B` 每月自动流向缺口更大的标的。回撤部署只认 **SPYM** 的回撤(≥10/25/35%,v4.3 起首档为 10%),动用的是常态下限以下的危机弹药(共 6pp of NAV)。实测:QQQM 近两年三次 11–14% 回撤发生时 SPYM 仅 3.9%–9.1%,均未达档。见 `Research/2026-08-01-drawdown-vs-rebalancing-scope.md` 与 `2026-08-01-t1-threshold-10pct.md`。
